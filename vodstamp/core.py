...
def video_id(url):
    ...
    if not re.fullmatch(r'[A-Za-z0-9_-]{11}', value):
        raise ValueError('Enter a valid HTTPS YouTube URL.')
    return value
...
def manual_range(start, zone, hours):
    try:
        ...
        raise ValueError('Date: YYYY-MM-DD HH:MM:SS. Duration must be greater than 0 and at most 744 hours.') from None
    if dt.tzinfo is None:
        try:
            tz = ZoneInfo(zone)
        except ZoneInfoNotFoundError:
            raise ValueError('Timezone unavailable. Use an explicit UTC offset or install tzdata.') from None
        ...
        if len(candidates) != 1:
            raise ValueError('Ambiguous or nonexistent local time at a daylight-saving transition. Specify an explicit UTC offset.')
        dt = candidates.pop()
...
def match_row(match, owners, start, end, offset=0):
    ...
    note = 'Multiple accounts present: check perspective.' if len(owned) > 1 else ''
    ...
    if info.get('mapId') == 11 and mode == 'CLASSIC':
        ...
        if role in ROLES and len(rivals) == 1:
            ...
            note = (note + ' Riot-estimated role; check for lane swaps.').strip()
        else:
            title = f'{champion} vs ?'
            note = (note + ' Opponent unknown: edit the title.').strip()
    else:
        title = f'{champion} - {mode}'
    seconds = int((when - start).total_seconds()) + offset
    if seconds < 0:
        note = (note + ' Negative offset: timestamp clamped to zero.').strip()
